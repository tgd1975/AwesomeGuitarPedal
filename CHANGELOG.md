...
**Mobile App (Flutter, EPIC-023)**

- Info/About page with version, license, and project links (TASK-331)
- How-To quickstart page — 3–5 steps, English-only static content (TASK-332)
- Profiles explainer, Troubleshooting, and Legal pages (TASK-333)
- Startup splash screen — Flutter + native Android (TASK-334)
- Connection status strip and pedal-details bottom sheet (TASK-335)
- Connected-Pedal page (board view) (TASK-336)
- HID display table (auto-generated) and Live-Keystroke page (TASK-338)
- App reads and displays unified firmware version via BLE READ characteristic `516515c5` (TASK-349, TASK-354)

**Branding (EPIC-024)**

- ASP mark + favicon on web simulator (TASK-339), profile builder (TASK-340), and configuration builder (TASK-341)
- ASP mark in mobile AppBar (TASK-342)
- Android launcher icon swapped to ASP monogram with adaptive icon (TASK-345)
- GitHub social preview image with og:image meta tags (TASK-343)

**Web Tools**

- Unified version stamp on simulator and both builders (TASK-350)
- Simulator boots with demo profiles loaded; community gallery still reachable (TASK-303)

**Firmware**

- ESP32 exposes firmware version via dedicated BLE READ characteristic `516515c5` (TASK-354)
- Profile-independent actions — actions that fire regardless of active profile (firmware + schema, TASK-306)

**Configuration / Schema**

- Profile-independent actions wired into web simulator, web profile builder, and Flutter app (TASK-307, TASK-308, TASK-309)

**Release / CI**

- Android APK built and attached to GitHub Releases via `release.yml` (TASK-179)
- CI merge gate on `main`: required checks must pass, branch must be up-to-date, linear history enforced (TASK-362)
- `clang-tidy` promoted to required CI gate; static-analysis backlog cleared (TASK-371)
- `platformio.ini` linted in pre-commit and CI (TASK-361)
- Pre-commit hook content-sensitive: C++ checks only run when C++ changed (TASK-346)
- `clang-tidy` lint set derived from `compile_commands.json` (TASK-348)
- Autogenerated markdown is lint-clean by construction (TASK-351)

**Commit policy (EPIC-022)**

- `/commit` skill uses git pathspec form for atomic commits and parallel-session safety (TASK-323)
- Pre-commit hook enforces `/commit`-only commits via provenance token (TASK-322, TASK-325)
- `COMMIT_POLICY.md` documents the rationale, bypass mechanism, and bypass log (TASK-324)
- `/release` and `/release-branch` use the documented `ASP_COMMIT_BYPASS` bypass (TASK-326)
- `/commit` correctly handles renames and deletions (TASK-347)
- Auto-`git-add` for untracked files removed from `commit-pathspec.sh` (TASK-329)

**Task system**

- Emoji icons + non-breaking space for idea categories in OVERVIEW (TASK-305, TASK-313, TASK-314)
- Idea sub-file naming convention: one IDEA = one OVERVIEW row (TASK-315)
- Optional `branch:` field on epic frontmatter; `/ts-epic-new` auto-fills `feature/<epic-slug>` (TASK-316, TASK-317)
- `/ts-task-active` warns on epic branch mismatch (TASK-318)
- Parallel `housekeep.py --apply` execution prevented via lockfile (TASK-328)
- EPIC-025 nrf52840-blocked + `/nrf5-task-routing` skill route nRF52840 work while hardware unavailable

**Developer tooling**

- `/claude-recap` — automated post-session transcript review (renamed from claude-review)
- Security review of incoming pulls
- Android app test protocol records device + Android version per run (TASK-259)

### Changed

- `/release` skill thinned — firmware build and `gh release create` moved into `release.yml` (TASK-364)
- Release flow requires branch protection: `--squash` or `--rebase` only (linear history)

### Fixed

- CI Mermaid lint, `clang-tidy`, and dirty-push prevention reconciled with local hooks (TASK-321)
- MD025 violations swept across `docs/`; pre-commit hook glob fixed (TASK-369)
- Simulator pedal buttons no longer react to mouse hover (TASK-304)
- `MAX_CONFIG_BYTES` (16 KB) reconciled between BLE protocol doc and reassembler header (TASK-357)
- nRF52840 on-device build no longer pulls in `test/fakes` includes
